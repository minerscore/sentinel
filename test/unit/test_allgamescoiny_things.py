...
def test_valid_sov_address():
    from sovlib import is_valid_sov_address

    main = valid_sov_address()
    test = valid_sov_address('testnet')

    assert is_valid_sov_address(main) is True
    assert is_valid_sov_address(main, 'mainnet') is True
    assert is_valid_sov_address(main, 'testnet') is False

    assert is_valid_sov_address(test) is False
    assert is_valid_sov_address(test, 'mainnet') is False
    assert is_valid_sov_address(test, 'testnet') is True


def test_invalid_sov_address():
    from sovlib import is_valid_sov_address

    main = invalid_sov_address()
    test = invalid_sov_address('testnet')

    assert is_valid_sov_address(main) is False
    assert is_valid_sov_address(main, 'mainnet') is False
    assert is_valid_sov_address(main, 'testnet') is False

    assert is_valid_sov_address(test) is False
    assert is_valid_sov_address(test, 'mainnet') is False
    assert is_valid_sov_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from sovlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % sovlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(sovlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(sovlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(sovlib.blocks_to_seconds(16616)) == 2612035
