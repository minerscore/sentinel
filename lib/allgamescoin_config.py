...
class RavenDarkConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read ravendark.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is ravendark.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from ravendark.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard RavenDark defaults...
        default_port = 7207 if (network == 'mainnet') else 17207

        # use default port for network if not specified in ravendark.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from ravendark.conf
